/*
 * Vendored from OpENer's POSIX sample application
 * (third_party/OpENer/source/src/ports/POSIX/sample_application/opener_user_conf.h).
 *
 * A copy rather than an include path into the submodule, because this file is
 * the stack's compile-time configuration - connection counts, buffer sizes,
 * which optional CIP objects exist - and those are ours to set, not upstream's.
 * Pointing at the sample's copy would mean a submodule bump could silently
 * change the device profile.  Keep this in sync when the submodule moves.
 */
/*******************************************************************************
